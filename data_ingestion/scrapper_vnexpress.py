...
def vne_fetch_article_urls(base_url='https://vnexpress.net/kinh-doanh/chung-khoan-p', max_pages=3):
    """Lấy danh sách URL của các bài báo từ trang chính."""
    urls = []
    # Số lượng trang muốn cào (cứ mỗi 15 là 1 trang mới)
    for offset in range(1, 6, 1):  # Ví dụ: 4 trang đầu tiên
        curr_url = base_url + str(offset)
        print(f"Đang duyệt trang: {curr_url}")
        urls.extend(vne_get_article_links(curr_url))

    return urls

def vne_scrape_article_content(url):
